...
> What `types of weapons` do the assailants' likely use? 

> What conclusion about the `age of shooter`, `race` and `prior mental health issues` could we draw out?
...
- Those are the outliers for each of the variables that they represent.
...
There is information’s limitation about the observations in `location` variable in the raw data. - Most of the Nightlife observations like Bar/Club/Restaurants was classified as `Other`, which make the original data had a big chunk of number in 'Other' category.
